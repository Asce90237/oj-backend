...
public class QuestionServiceImpl extends ServiceImpl<QuestionMapper, Question>
    ...
    @Override
    public QuestionVO getQuestionVO(Question question, HttpSession session) {
        QuestionVO questionVO = QuestionVO.objToVo(question);
        // 判断是否提交答案是否成功过，若成功过则可以返回答案
        User loginUser = userFeignClient.getLoginUser(session);
        boolean b = questionSubmitMapper.userQuestionAccepted(loginUser.getId(), question.getId());
        if (!b) {
